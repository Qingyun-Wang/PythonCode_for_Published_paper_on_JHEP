...
def traceMinimum(f, d2f_dxdt, d2f_dx2, x0, t0, tstop, dtstart, deltaX_target,
                 ...
                 deltaX_tol=1.2, minratio=1e-5):                                       #change deltaX_tol from 1.2 to 25. these                                                                       #quantity is esential in determing the end of one phase
                                                                        # change  minratio=1e-2 to 1e-5
    """
    ...
    ################# set start time
    start_time = time.time()                                              ####label the beginning time
    ...
    while dxdt is not None:
        ...
        ####################### quiet when it take too much time
        current_time=time.time()
       
    #    if current_time-start_time>600:
    #        print("It takes more than 15mins to trace the phases, so we quit here")
    #        break                                     ##### quit when it takes too long
        ################################
...
class Phase:
    ...
    def addLinkFrom(self, other_phase):
        ...
        """
        if np.min(self.T) >= 0.999*np.max(other_phase.T):                                #########add 0.999
            self.low_trans.add(other_phase.key)
            other_phase.high_trans.add(self.key)
        if np.max(self.T) <= 1.001*np.min(other_phase.T):                                #########add 1.001
...
def traceMultiMin(f, d2f_dxdt, d2f_dx2,
                  ...
                  dtstart=1e-3, tjump=1e-3, forbidCrit=None,                             ###change tjump from e-3 to e-2####
    ...
    while len(nextPoint) != 0:
        ...
        for i in phases.keys():
            ...
            if np.sum((x-x1)**2)**.5 < 2*deltaX_target:               ###maybe change the right of the inequality to some larger
                # The point is already covered                            value which related to x or derltaX_target?
      ...
      #      if current_time-start_time>600:
      #          print("It takes more than 15mins to trace the phases, so we quit here")
      #          return phases                                                ##### quit when it takes too long
...
def _tunnelFromPhaseAtT(T, phases, start_phase, V, dV,
    ...
    for key in phases.keys():
        if key == start_phase.key:
            continue
        p = phases[key]
        if (p.T[0] > T or p.T[-1] < 1.001*T):                                  ####################change T to 1.001T
            continue
        x1 = fmin(p.valAt(T))
        V1 = V(x1, T)
        if V1 >= V0:
            ...
            if nuclCriterion(outdict[Tmin]['action'], Tmax) < 0:
                ...
                try:         
                    Tnuc = optimize.brentq(
                        _tunnelFromPhaseAtT, Tmin, Tmax,                         #make Tnuc into try and except environment
...
def findCriticalTemperatures(phases, V, start_high=False):
    ...
    for i in phases.keys():
        for j in phases.keys():
            ...
            tmax = min(phase1.T[-1], phase2.T[-1])
            tmin = max(phase1.T[0], phase2.T[0])
            if tmin >= 0.999*tmax:                                                   #change from tmax to 0.999tmax
